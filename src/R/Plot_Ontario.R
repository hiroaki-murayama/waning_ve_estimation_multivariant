...
result[1:50,] %>% cbind(week,lower_ref,median_ref,upper_ref) -> others_ve
result[(t1+1):(t1+50),] %>% cbind(week,lower_ref,median_ref,upper_ref) -> alpha_ve
result[(t2+1):(t2+50),] %>% cbind(week,lower_ref,median_ref,upper_ref) -> delta_ve
...
others_plot <- others_ve %>% ggplot() +
  ...
  theme(axis.text.x = element_text(size=15, angle = 0, hjust = 1),axis.text.y = element_text(size=15),
        ...
        legend.position = "right") + scale_x_continuous(breaks = seq(1, 50, 4), limits=c(1, 50)) + 
...
delta_plot <- delta_ve %>% ggplot() +
  ...
  theme(axis.text.x = element_text(size=15, angle = 0, hjust = 1), axis.text.y = element_text(size=15),
        ...
        axis.text = element_text(size=10, family="sans",color="black"),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  scale_x_continuous(breaks = seq(1, 50, 4), limits=c(1, 50)) +
  scale_y_continuous(breaks = seq(0, 100, 10), limits=c(0, 100)) +theme(legend.position=c(.3,.2)) +       
